    ...
    function start() {
        ...
        $('#timer').prepend(`<h2> Time Remaining: <span id="counter-number"> ${counter}</span>  Seconds</h2>`);

        for (var i = 0; i < questions.length; i++) {
            $("#mainBody").append('<h2>' + questions[i].question + '</h2>');
            for (var j = 0; j < questions[i].answers.length; j++) {
                $("#mainBody").append('<input type="radio" name="question' + '-' + i + '" value="' + questions[i].answers[j] + '">' + questions[i].answers[j]);
            }
        }

        $("#donebtn").append('<button id="done">Done</button>');
    }
    function countdown() {
        counter--;
    ...
    function done() {
        ...
        $("#mainBody").empty();
        $('#timer h2').remove();
        $('#mainBody').append('<h3>Correct Answers: ' + correct + '</h3>');
        $('#mainBody').append('<h3>Incorrect Answers: ' + inCorrect + '</h3>');
        $('#mainBody').append('<h3>Unanswered: ' + (questions.length - (inCorrect + correct)) + '</h3>');
    }
